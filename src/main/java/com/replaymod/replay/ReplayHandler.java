...
public class ReplayHandler {
    ...
    //$$     if (cam != null) {
    //$$         targetCameraPosition = new Location(Entity_getX(cam), Entity_getY(cam), Entity_getZ(cam), cam.rotationYaw, cam.rotationPitch);
    //$$     } else {
    ...
    public boolean isCameraView() {
        return mc.thePlayer instanceof CameraEntity && mc.thePlayer == getRenderViewEntity(mc);
    }
    ...
    public void doJump(int targetTime, boolean retainCameraPosition) {
        ...
        //$$     for (Entity entity : loadedEntityList(mc.theWorld)) {
        //$$         skipTeleportInterpolation(entity);
        //$$         entity.lastTickPosX = entity.prevPosX = Entity_getX(entity);
        //$$         entity.lastTickPosY = entity.prevPosY = Entity_getY(entity);
        //$$         entity.lastTickPosZ = entity.prevPosZ = Entity_getZ(entity);
        ...
        if (diff != 0) {
            if (diff > 0 && diff < 5000) { // Small difference and no time travel
                ...
                //#else
                //#if MC>=11400
                //$$ getWindow(mc).loadGUIRenderMatrix(true);
                //#else
                //#if MC>=11400
                ...
                pushMatrix();
                //#if MC>=11400
                //$$ mc.getFramebuffer().framebufferRender(getWindow(mc).getFramebufferWidth(), getWindow(mc).getFramebufferHeight());
                //#else
